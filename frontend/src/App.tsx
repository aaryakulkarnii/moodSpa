...
const BG_DARK =
  "radial-gradient(125% 125% at 50% 101%, rgba(60,10,40,1) 10%, rgba(40,10,60,1) 25%, rgba(18,12,35,1) 65%, rgba(8,8,20,1) 100%)";

export default function App() {
  const { messages, isLoading, send, clear, ready } = useChat();
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col w-full max-w-2xl mx-auto">
        ...
        <AnimatePresence mode="wait">
          {!ready ? (
            /* Loading spinner while history fetches */
            <motion.div
              key="loading"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        {ready && <PromptInputBox onSend={send} disabled={isLoading} />}
      </div>
    </div>
  );
}
